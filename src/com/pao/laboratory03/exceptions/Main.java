...
public class Main {
    ...
    // Method that validates age and throws InvalidAgeException
    public static void validateAge(int age) {
        if (age < 0 || age > 150) {
            throw new InvalidAgeException("Varsta " + age + " nu este valida (0-150)");
        }
        System.out.println("Varsta " + age + " este valida");
    }
    
    // Method that adds to list and throws DuplicateEntryException
    public static void addToList(List<String> list, String name) {
        if (list.contains(name)) {
            throw new DuplicateEntryException("'" + name + "' exista deja în lista");
        }
        list.add(name);
        System.out.println("'" + name + "' a fost adăugat în lista");
    }
    
    // Method with throws in signature
